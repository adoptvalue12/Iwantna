...
local function FireEmoteFling()
    ...
    if EmoteFlingConfig.Resolving then
        Notify({ Title = "Kevin DropKick", Content = "Please wait, resolving animation...", Duration = 2, Icon = "loader" })
        return
    end
    ...
    if not EmoteFlingConfig.Armed then
        Notify({ Title = "Kevin DropKick", Content = "Fling is not armed. Enable it first.", Duration = 2, Icon = "alert-circle" })
        return
    end

    local targetId = EmoteFlingConfig.AnimId
    if type(targetId) ~= "string" or targetId == "" or targetId:match("^%s*$") then
        Notify({ Title = "Kevin Dropkick", Content = "No valid Animation ID set.", Duration = 2, Icon = "alert-circle" })
        return
    end
    ...
    if not (hrp and hum) then
        Notify({ Title = "Kevin DropKick", Content = "Character not found.", Duration = 2, Icon = "alert-circle" })
        return
    end

    EmoteFlingConfig.Resolving = true
    Notify({ Title = "Kevin DropKick", Content = "Resolving animation...", Duration = 1.5, Icon = "loader" })
    ...
    task.spawn(function()
        ...
        if not finalAnimId then
            Notify({ Title = "Kevin DropKick", Content = "Invalid ID! Ensure it's an Animation or Emote.", Duration = 3, Icon = "x-circle" })
            return
        end
        ...
        if not trackOk or not track then
            Notify({ Title = "Kevin DropKick", Content = "Failed to load animation track.", Duration = 2, Icon = "x-circle" })
            return
        end
        ...
        Notify({ Title = "Kevin DropKick", Content = "Active! Press Fling Keybind or execute again to STOP.", Duration = 2, Icon = "zap" })
        ...
        EmoteFlingCleanup()
        Notify({ Title = "Kevin DropKick", Content = "Physics restored to normal.", Duration = 2, Icon = "shield-off" })
    end)
end

VaporLens:SetTheme({

	Glass = Color3.fromRGB(20, 8, 8),
	GlassTransp = 0.35,
	...
	Primary = Color3.fromRGB(255, 235, 235),
	Secondary = Color3.fromRGB(185, 145, 145),
	SecTransp = 0,

	ElemBg = Color3.fromRGB(35, 15, 15),
	ElemTransp = 0.50,
	ElemHoverTransp = 0.25,
	ElemBdrTransp = 0.70,

	ToggleOff = Color3.fromRGB(50, 25, 25),
	SliderTrack = Color3.fromRGB(25, 10, 10),
	InputBg = Color3.fromRGB(14, 6, 6),

	SectionTransp = 0.35,

	NotifBg = Color3.fromRGB(10, 4, 4),
})

local uiWindow = VaporLens:CreateWindow({
    Title = "Kevin DropKick",
    Subtitle = "discord.gg/K92JgMUnCG",
    Icon = "plane",
    Width = 340,
    Height = 520,
...
local flingTab = uiWindow:CreateTab({ Title = "Kevin DropKick", Icon = "zap" })

flingTab:CreateSection("Settings")

flingTab:CreateToggle({
    Name = "Enable DropKick",
...
local function OnAntiFlingToggle(Value, isInitialization)
    ...
    if EmoteFlingConfig.antiFlingEnabled then
            ...
            if hrp and humanoid and humanoid.Health > 0 then
                ...
                if isFlinged then
                    ...
                    if tick() - lastNotifyTimeFling > 2.0 then
                        Notify({Title = "Anti-Fling", Content = "Anomalia cinĂ©tica suprimida.", Duration = 1.5, Icon = "shield"})
        ...
        if not isInitialization then
            Notify({Title = "Anti-Fling", Content = "ProteĂ§Ă£o Absoluta e Fly Bypass sincronizados.", Duration = 2.5, Icon = "check-circle"})
        end
    else
        DisconnectConnection("antiFlingLoopConn")
...
local connCharAdded = LocalPlayer.CharacterAdded:Connect(function(newChar)
    if EmoteFlingConfig.Active then
        EmoteFlingConfig.Active = false
        Notify({ Title = "Kevin DropKick", Content = "Interrupted (respawn).", Duration = 2, Icon = "shield-off" })
    end
end)
...
if ConfigLoaded then
    Notify({ Title = "Kevin DropKick", Content = "Config loaded and synced.", Duration = 2, Icon = "check-circle" })
else
    Notify({ Title = "Kevin DropKick", Content = "Module loaded (Defaults).", Duration = 3, Icon = "check-circle" })
end
